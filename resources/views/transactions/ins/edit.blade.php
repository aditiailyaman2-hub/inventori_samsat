<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Barang Masuk</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('transactions.ins.update', $itemIn) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="item_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach($items as $item)
                                <option value="{{ $item->id }}" @selected($item->id === $itemIn->item_id)>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="jumlah_masuk" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" min="1" value="{{ old('jumlah_masuk', $itemIn->jumlah_masuk) }}" required>
                        @error('jumlah_masuk')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Tanggal Masuk</label>
                        <input type="date" name="tanggal_masuk" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('tanggal_masuk', $itemIn->tanggal_masuk) }}" required>
                        @error('tanggal_masuk')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700">Keterangan</label>
                        <input type="text" name="keterangan" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('keterangan', $itemIn->keterangan) }}">
